...
- **ORGANIZATIONS**: Stores organization_id (UUID PK), name, country.
- **ASSESSMENTS**: Stores assessment_id (UUID PK), user_id (VARCHAR, Keycloak sub), data (JSONB for answers), status (ENUM: Draft, Submitted, Completed).
- **QUESTIONS**: Stores question_id (UUID PK), text (JSONB for multilingual), category, weight.
- **ASSESSMENT_QUESTIONS**: Junction table with assessment_id, question_id (PK, FK), answer (JSONB).
...
erDiagram
    ...
    QUESTIONS ||--o{ ASSESSMENT_QUESTIONS : "included in"
