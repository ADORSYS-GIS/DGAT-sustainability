...
export const useReviewAssessments = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'under_review':
        return { variant: 'secondary', className: 'bg-yellow-100 text-yellow-800', text: t('reviewAssessments.underReview', { defaultValue: 'Under Review' }) };
      case 'approved':
        return { variant: 'default', className: 'bg-green-100 text-green-800', text: t('reviewAssessments.approved', { defaultValue: 'Approved' }) };
      case 'rejected':
        return { variant: 'destructive', className: '', text: t('reviewAssessments.rejected', { defaultValue: 'Rejected' }) };
      default:
