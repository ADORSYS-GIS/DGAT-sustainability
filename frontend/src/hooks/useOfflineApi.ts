  ...
  const deleteQuestion = useCallback(
    async (
      ...
      try {
        ...
        if (existingQuestion) {
          // Question exists locally, get the latest revision ID
          latestRevisionId =
          ...
          // Question doesn't exist locally, try to get it from the server first
          console.warn(
            `Question ${questionId} not found locally, attempting server deletion`,
          );

          // Try to get the question from the server to get the revision ID
          try {
            const { QuestionsService } = await import(
              "@/openapi-rq/requests/services.gen"
            );
            const serverQuestion =
              await QuestionsService.getQuestionsByQuestionId({
                questionId,
              });

            if (
              serverQuestion?.question?.latest_revision?.question_revision_id
            ) {
              latestRevisionId =
                serverQuestion.question.latest_revision.question_revision_id;
            } else {
              throw new Error(
                "Question not found on server or has no revision",
              );
            }
          } catch (serverError) {
            console.error("Failed to get question from server:", serverError);
            throw new Error("Question not found locally or on server");
          }
        }
  ...
  const deleteCategory = useCallback(
    async (
      ...
      try {
        ...
        if (!categoryToDelete) {
          // Category doesn't exist locally, try to delete from server anyway
          console.warn(
            `Category ${categoryId} not found locally, attempting server deletion`,
          );

          // Try to get the category from the server first
          try {
            const { CategoriesService } = await import(
              "@/openapi-rq/requests/services.gen"
            );
            const serverCategories = await CategoriesService.getCategories();
            const serverCategory = serverCategories.categories?.find(
              (cat) => cat.category_id === categoryId,
            );

            if (serverCategory) {
              categoryToDelete = {
                category_id: serverCategory.category_id,
                name: serverCategory.name,
                weight: serverCategory.weight,
                order: serverCategory.order,
                template_id: serverCategory.template_id,
                created_at: serverCategory.created_at,
                updated_at: serverCategory.updated_at,
                sync_status: "synced",
                local_changes: false,
            ...
            console.error("Failed to get category from server:", serverError);
            throw new Error("Category not found locally or on server");
          }
        }
  ...
  const submitAssessment = useCallback(
    async (
      ...
      try {
        ...
        try {
          const offlineSubmission =
            DataTransformationService.transformSubmission(
              tempSubmissionForTransform,
            );
          await offlineDB.saveSubmission(offlineSubmission);
        } catch (storageError) {
          console.error(
            "❌ Failed to store submission in IndexedDB:",
            storageError,
          );
        ...
        const error =
          err instanceof Error ? err : new Error("Failed to submit assessment");
        options?.onError?.(error);
        throw error;
      } finally {
        setIsPending(false);
