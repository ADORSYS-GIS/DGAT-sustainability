...
import { Organization, User, Assessment } from "@/services/indexedDB";
...
export const AdminDashboard: React.FC = () => {
  ...
  React.useEffect(() => {
    const fetchStatsAndReviews = async () => {
      try {
        const [orgs, users, assessments] = await Promise.all([
          getAllOrganizations(),
          getAllUsers(),
          getAllAssessments(),
        ]);
        ...
        setPendingCount(pendingAssessments.length);

        setCompletedCount(
          assessments.filter((a: Assessment) => a.status === "completed")
            .length,
        );

        const orgMap = new Map(
          orgs.map((o: Organization) => [o.organizationId, o.name]),
        );
        const userMap = new Map(
          users.map((u: User) => [u.userId, `${u.firstName} ${u.lastName}`]),
        );

        const reviewsData = pendingAssessments.map(
          (assessment: Assessment) => ({
            ...
            type: (assessment.templateId || "").toLowerCase().includes("dgat")
              ? "DGAT"
              : "Sustainability",
            submittedAt: new Date(assessment.createdAt).toLocaleDateString(
              "en-CA",
            ),
          }),
        );
