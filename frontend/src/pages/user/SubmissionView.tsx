...
export const SubmissionView: React.FC = () => {
  ...
  const findResponseForQuestion = (question_revision_id: string, revision: QuestionRevision) => {
    const responses = submission?.content?.responses as SubmissionResponse[] | undefined;
    if (!responses) return undefined;
    // Try to match by question_revision_id first
    let found = responses.find((r) => r.question_revision_id === question_revision_id);
    ...
    if (typeof revision.text === 'object' && revision.text !== null) {
      ...
      if (typeof textObj['en'] === 'string') {
        ...
        const firstString = Object.values(textObj).find((v) => typeof v === 'string');
    ...
    function hasQuestionEn(obj: unknown): obj is { question: { en: string } } {
      return (
        ...
        typeof ((obj as { question: { en?: unknown } }).question as { en?: unknown }).en === 'string'
      );
    }
  ...
  const renderReadOnlyAnswer = (revision: QuestionRevision, response: SubmissionResponse | undefined) => {
    // Parse the answer
    ...
    try {
      ...
      answer = Array.isArray(response?.response) ? response?.response[0] : response?.response;
    }
    // Extract values
    const yesNoValue = typeof answer === 'object' && answer !== null && 'yesNo' in answer ? (answer as { yesNo?: boolean }).yesNo : undefined;
    const percentageValue = typeof answer === 'object' && answer !== null && 'percentage' in answer ? (answer as { percentage?: number }).percentage : undefined;
    const textValue = typeof answer === 'object' && answer !== null && 'text' in answer ? (answer as { text?: string }).text : '';
    // Files are inside the answer object, not the top-level response
    const files: { name?: string; url?: string }[] = typeof answer === 'object' && answer !== null && Array.isArray((answer as { files?: { name?: string; url?: string }[] }).files) ? (answer as { files: { name?: string; url?: string }[] }).files : [];
    return (
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex space-x-4 mt-1">
            <Button
              ...
              className={yesNoValue === true ? "bg-dgrv-green text-white border-dgrv-green" : "bg-white text-dgrv-green border-dgrv-green hover:bg-dgrv-green/10"}
              tabIndex={-1}
            ...
            <Button
              ...
              className={yesNoValue === false ? "bg-red-500 text-white border-red-500" : "bg-white text-red-500 border-red-500 hover:bg-red-500/10"}
              tabIndex={-1}
              style={{ pointerEvents: 'none', opacity: 1 }}
            >
              No
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <Card className="shadow-md bg-white/90 border-0">
            <CardHeader className="border-b pb-2 mb-2">
              <CardTitle className="text-2xl font-bold text-dgrv-blue tracking-tight text-left">Submission Overview</CardTitle>
            </CardHeader>
              ...
              <div className="flex flex-col md:flex-row md:space-x-8 space-y-2 md:space-y-0">
                <div className="flex-1">
                  ...
                  <span className="inline-block px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold">
                    {submission.review_status.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}
                  </span>
                </div>
                <div className="flex-1">
                  <span className="block text-xs text-gray-500 font-medium mb-1">Submitted At</span>
                  <span className="text-sm text-gray-700">{new Date(submission.submitted_at).toLocaleString()}</span>
                </div>
                {submission.reviewed_at && (
                  <div className="flex-1">
                    <span className="block text-xs text-gray-500 font-medium mb-1">Reviewed At</span>
                    <span className="text-sm text-gray-700">{new Date(submission.reviewed_at).toLocaleString()}</span>
                  </div>
                )}
              </div>
                ...
                <Accordion type="multiple" className="w-full divide-y divide-gray-100">
                  {categories.flatMap((category, catIdx) => [
                    <div key={category} className="text-left px-2 py-4">
                      <div className="text-lg font-semibold text-dgrv-blue mb-2">{category}</div>
                    </div>,
                    ...groupedQuestions[category].map(({ question, revision }, idx) => {
                      const questionText = getQuestionText(revision);
                      const response = findResponseForQuestion(getRevisionKey(revision), revision);
                      return (
                        <AccordionItem key={getRevisionKey(revision)} value={getRevisionKey(revision)} className="bg-white/80">
                          <AccordionTrigger className="text-left text-base font-medium text-gray-900 px-6 py-4 hover:bg-dgrv-blue/5 focus:bg-dgrv-blue/10 rounded-md justify-start items-start">
                            <span className="mr-2 text-gray-400">{idx + 1}.</span> {questionText}
                          </AccordionTrigger>
