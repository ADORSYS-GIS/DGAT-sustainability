...
const FileDisplay: React.FC<FileDisplayProps> = ({ files, title = 'Attachments' }) => {
  ...
  const openImageInNewTab = (file: { name?: string; url?: string; type?: string; [key: string]: unknown }) => {
  ...
  const isImageFile = (file: { name?: string; url?: string; type?: string; [key: string]: unknown }) => {
    if (!file.url) return false;
    return file.url.startsWith('data:image/') || 
           file.url.startsWith('http') && 
           (file.url.includes('.jpg') || file.url.includes('.jpeg') || 
            file.url.includes('.png') || file.url.includes('.gif') || 
            file.url.includes('.webp') || file.url.includes('.svg'));
  };
