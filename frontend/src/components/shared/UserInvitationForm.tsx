...
export const UserInvitationForm: React.FC<UserInvitationFormProps> = ({
  ...
  const [createdInvitation, setCreatedInvitation] =
    useState<UserInvitationResponse | null>(null);
  ...
  const triggerVerificationMutation = useMutation({
    mutationFn: async (userId: string) => {
      return AdminService.postAdminUserInvitationsByUserIdTriggerVerification({
        userId,
      });
    },
    onSuccess: () => {
  ...
  const triggerOrgInvitationMutation = useMutation({
    mutationFn: async (userId: string) => {
      return AdminService.postAdminUserInvitationsByUserIdTriggerOrgInvitation({
        userId,
      });
    },
    onSuccess: () => {
  ...
  const checkAndTriggerMutation = useMutation({
    mutationFn: async (userId: string) => {
      return AdminService.postAdminUserInvitationsByUserIdCheckAndTrigger({
        userId,
      });
    },
    onSuccess: (result) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {defaultOrganizationId ? (
              ...
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={t("userInvitation.selectOrganization")}
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500">
              DRGV Admin can only create Organization Admin users
            </p>
          </div>
