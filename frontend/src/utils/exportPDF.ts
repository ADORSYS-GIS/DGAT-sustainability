...
const addHeader = (doc: jsPDF) => {
  ...
  doc.text(
    pageText,
    doc.internal.pageSize.width - PAGE_MARGIN - doc.getTextWidth(pageText),
    10,
  );
...
export async function exportAllAssessmentsPDF(
  ...
  doc.text("SUSTAINABILITY REPORT 2025", pageWidth / 2, 175, {
    align: "center",
  });
  ...
  if (radarChartDataUrl) {
    ...
    doc.text(
      doc.splitTextToSize(radarIntro, pageWidth - PAGE_MARGIN * 2),
      PAGE_MARGIN,
      30,
    );
  ...
  if (recommendationChartDataUrl) {
    ...
    doc.text(
      doc.splitTextToSize(recIntro, pageWidth - PAGE_MARGIN * 2),
      PAGE_MARGIN,
      30,
    );
  ...
  doc.setTextColor(0, 0, 0);
  const kanbanIntro =
    "This Kanban board provides a visual tool to track the progress of each recommendation. Tasks are organized by their current status, from 'To Do' to 'Approved', facilitating effective project management.";
  doc.text(
    doc.splitTextToSize(kanbanIntro, pageWidth - PAGE_MARGIN * 2),
    PAGE_MARGIN,
    30,
  );
