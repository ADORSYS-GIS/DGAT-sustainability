...
class OfflineTestSuite {
  ...
  async runTest(name: string, testFn: () => Promise<void>): Promise<void> {
    ...
    try {
      ...
      console.error(
        `❌ ${name} - FAILED: ${error instanceof Error ? error.message : "Unknown error"} (${Date.now() - startTime}ms)`,
      );
    }
  }
  ...
  async runAllTests(): Promise<void> {
    ...
    await this.runTest("Conflict Detection", async () => {
      ...
      const latest =
        await offlineDB.getLatestResponsesByAssessment("test-assessment-1");
      const conflictResponse = latest.find(
        (r) => r.response_id === "conflict-test-1",
      );
    ...
    await this.runTest("IndexedDB Performance Test", async () => {
      ...
      console.log(
        `   📊 Performance: Saved and retrieved 100 questions in ${duration}ms`,
      );
    });
...
if (
  ...
  window.location.search.includes("test=offline")
) {
